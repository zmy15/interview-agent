...
| Mode | Description |
|------|-------------|
| 🕴️ **Interviewer Mode** | Simulates a professional interviewer — generates targeted questions from job descriptions & knowledge bases, evaluates candidate responses, and produces interview reports |
| 🧑‍💻 **Candidate Mode** | Simulates real interview scenarios to help job seekers practice interview skills and adapt to target role expectations |
...
```
Create Position → Add JD → Upload Knowledge → Upload Resume → Start Interview → Chat → Generate Report
```
...
4. Switch to "Interview" page, select interviewer mode and start
5. During the chat, the AI automatically retrieves relevant knowledge and asks targeted questions
6. Generate a multi-dimensional evaluation report when finished

### Candidate Mode

```
Create Position → Add JD → Start Mock Interview → Practice
```

1. Create a target position with its JD
2. Switch to candidate mode — the AI simulates a real interview for that role
3. Improve interview skills through repeated practice
