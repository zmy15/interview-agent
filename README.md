...
| 模式 | 说明 |
|------|------|
| 🕴️ **面试官模式** | 模拟专业面试官，根据岗位 JD 和知识库自动生成针对性问题，评估候选人回答并生成面试报告 |
| 🧑‍💻 **求职者模式** | 模拟真实面试流程，帮助求职者练习面试技巧，提前适应目标岗位的面试节奏 |
...
```
创建岗位 → 添加 JD → 上传知识库 → 上传简历 → 开始面试 → 实时对话 → 生成报告
```

1. 在「岗位管理」页面创建岗位，添加职位描述
2. 在「知识库」页面上传相关文档（技术规范、FAQ 等）
3. 在「上传」页面上传候选人简历（PDF / Word）
4. 切换到「面试」页面，选择面试官模式并开始
5. 在对话中，AI 会自动检索知识库，提出针对性问题
6. 结束后生成多维度面试评估报告

### 求职者模式

```
创建岗位 → 添加 JD → 开始模拟面试 → 练习回答
```

1. 创建目标岗位并填写 JD
2. 切换到求职者模式，AI 会模拟该岗位的真实面试流程
3. 通过反复练习提升面试技巧
